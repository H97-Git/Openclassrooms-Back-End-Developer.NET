...
namespace CalifornianHealth.Demographics.Data
{
    ...
    {
        public static void Initialize(DemographicsContext context)
        {
            Log.Information("DbInitializer : EnsureCreated()");
            context.Database.EnsureCreated();

            if (context.Consultants.Any() || context.Patients.Any())
            {
            ...
            foreach (var p in patients) context.Patients.Add(p);

            foreach (var c in consultants) context.Consultants.Add(c);
